...
const TimePageComponent = () => {
    ...
    const checkIsDateValid = () => {
        ...
        if(timezoneData.convertedStart && timezoneData.convertedEnd){
            const formatedStart = moment(convertedStart, "DD-MM-YYYY HH:mm").format("YYYY-MM-DDTHH:mm:ss")
            const formatedEnd = moment(convertedEnd, "DD-MM-YYYY HH:mm").format("YYYY-MM-DDTHH:mm:ss")
            console.log(formatedStart, formatedEnd);
            if(currentDate.isBetween(formatedStart, formatedEnd)){
                setEdit(true)
                console.log("yes");
            }
            else{
  ...
  return (
    <React.Fragment>
      <div className='h-screen w-screen flex justify-center items-center flex-col gap-10 px-80 container'>
        ...
        
        {
            ...
            <div className='w- container flex flex-col gap-5'>
                <div className='flex justify-between w-full'>
                    ...
                    {timezoneData.IST  && <p className='bg-gray-200 px-4 py-1 rounded-md flex gap-3'>{timezoneData.IST.convertedStart } <span>to</span> {timezoneData.IST.convertedEnd }</p>}
                </div>
                ...
                <div className='flex w-full justify-between'>
                    ...
                    {timezoneData.JST && <p className='bg-gray-200 px-4 py-1 rounded-md flex gap-3'>{timezoneData.JST.convertedStart } <span>to</span> {timezoneData.JST.convertedEnd }</p>} 
               </div>
                ...
                <div className='flex w-full justify-between'>
                    ...
                    {timezoneData.GMT && <p className='bg-gray-200 px-4 py-1 rounded-md flex gap-3'>{timezoneData.GMT.utcConvertedStart } <span>to</span> {timezoneData.GMT.utcConvertedEnd }</p>}
                ...
                <div className='flex w-full justify-between'>
                    ...
                    {timezoneData.Pacific && <p className='bg-gray-200 px-4 py-1 rounded-md flex gap-3'>{timezoneData.Pacific.convertedStart } <span>to</span> {timezoneData.Pacific.convertedEnd }</p>}
                </div>
        ...
        <div>
            <button disabled= {edit } className={` ring-0 text-white px-1 py-2 text-lg font-semibold rounded-lg w-28 ${edit ? "bg-green-500 hover:bg-green-400 " : "bg-gray-400 cursor-not-allowed" }`}>Edit</button>
